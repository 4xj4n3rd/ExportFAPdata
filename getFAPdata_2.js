...
function getCurrentClassName() {
    const classTable = document.getElementById('ctl00_mainContent_divGroup');
    const selectedClassElement = classTable.querySelector('b');

    if (selectedClassElement) {
        return selectedClassElement.textContent.trim().replace(/\./g, '_');
    } else {
        return 'default_class_name';  // fallback in case class name isn't found
    }
}
